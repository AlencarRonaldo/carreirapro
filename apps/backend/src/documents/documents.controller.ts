...
export class DocumentsController {
  constructor(
    ...
    @InjectRepository(TemplateFavoriteEntity)
    private readonly favs: Repository<TemplateFavoriteEntity>,
  ...
  async exportPdf(
    ...
    try {
      ...
      const template =
        SIMPLE_TEMPLATES.find((t) => t.key === doc.templateKey) ??
        SIMPLE_TEMPLATES[0];
      ...
      if (!isTestEnv && template.html && profile) {
        try {
          ...
          const html = template.html
            ...
            .replaceAll(
              '{{locationState}}',
              safe((profile as any).locationState),
            )
            .replaceAll(
              '{{locationCountry}}',
              safe((profile as any).locationCountry),
            )
            ...
            .replaceAll(
              '{{maritalStatus}}',
              safe((profile as any).maritalStatus),
            )
          ...
          // Base CSS ABNT (A4, margens 3/2cm, Times 12pt, 1.5, justificado)
          const baseCss = `<style>
      ...
      try {
        ...
        const cy = (pdf as any).page?.height
          ? (pdf as any).page.height / 2
          : 420;
        pdf.save();
      ...
      const body = (template.body || '{{content}}')
        ...
        .replaceAll(
          '{{experiences}}',
          xpList.map((x) => `• ${x.title} — ${x.company}`).join('\n'),
        )
        .replaceAll(
          '{{education}}',
          edList.map((e) => `• ${e.degree} — ${e.institution}`).join('\n'),
        )
        .replaceAll(
          '{{skills}}',
          skList.map((s) => `• ${s.name} (nível ${s.level})`).join('\n'),
        );
  ...
  @Get('templates/list')
  listTemplates() {
    return SIMPLE_TEMPLATES.map((t) => ({
      key: t.key,
      name: t.name,
      atsReady: (t as any).atsReady === true,
      premium: (t as any).premium === true,
    }));
  }
  ...
  upsertCustomTemplate(
    ...
    @Body() body: Partial<TemplateEntity>,
  ) {
  ...
  async previewTemplate(
    ...
    if (!tplBody && typeof tplHtml === 'undefined' && templateKey) {
      ...
      if (custom) {
        ...
        const simple =
          SIMPLE_TEMPLATES.find((t) => t.key === templateKey) ??
          SIMPLE_TEMPLATES[0];
        tplBody = simple.body ?? '';
    ...
    const replacements: Record<string, string> = {
      ...
      '{{linkedin}}': String((profile as any).linkedin ?? ''),
      ...
      '{{maritalStatus}}': String((profile as any).maritalStatus ?? ''),
      '{{content}}': content || '',
      '{{experiences}}': xpList
        .map((x) => {
    ...
    let htmlFromBody = tplHtml
      ? apply(tplHtml)
          .replace(
            '{{experiences}}',
            `<ul>${replacements['{{experiences}}']}</ul>`,
          )
    ...
    // Inject base CSS ABNT for preview também
    ...
    const unknownPlaceholders = Array.from(
      new Set(
        (htmlFromBody.match(/\{\{[^}]+\}\}/g) || []).filter(
          (p) => !Object.keys(replacements).includes(p),
        ),
      ),
    );
  ...
  async exportTemplatePdf(
    ...
    // Inject base CSS ABNT for export também
    ...
    pdf
      .fontSize(18)
      .text(String((profile as any).fullName ?? ''), { underline: true });
    pdf.moveDown();
