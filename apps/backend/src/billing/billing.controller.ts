...
export class BillingController {
  constructor(@InjectRepository(UserEntity) private readonly users: Repository<UserEntity>, private readonly stripe: StripeService) {}

  @UseGuards(JwtAuthGuard)
  @Post('checkout-session')
  async createCheckoutSession(@Req() req: any, @Body() body: { plan: 'free' | 'premium' | 'pro' }) {
    ...
    if (this.stripe.isConfigured()) {
      try {
        ...
        const res = await this.stripe.createCheckoutSession({ plan, customerEmail: user?.email, successUrl: `${process.env.APP_BASE_URL ?? ''}/?billing=success`, cancelUrl: `${process.env.APP_BASE_URL ?? ''}/?billing=canceled` });
  ...
  @UseGuards(JwtAuthGuard)
  @Get('mock/success')
  async mockSuccess(@Req() req: any, @Res() res: Response, @Query('plan') plan?: 'pro' | 'team') {
  ...
  @UseGuards(JwtAuthGuard)
  @Post('mock/activate')
  async mockActivate(@Req() req: any, @Body() body: { plan?: 'pro'|'team'; billingCycle?: 'monthly'|'yearly' }) {
    const userId: string = req.user?.sub;
    const user = await this.users.findOne({ where: { id: userId } });
    if (user) {
  ...
  async portal(@Req() req: any) {
    ...
    const res = await this.stripe.createPortalSession({ customerEmail: user.email, returnUrl: `${process.env.APP_BASE_URL ?? ''}/subscription` });
    return res ?? { url: '' };
  }
  ...
  private async handleCheckoutCompleted(event: any) {
    ...
    const plan: 'free' | 'premium' | 'pro' | undefined = session?.metadata?.plan;
  ...
  private async handleSubscriptionCreated(event: any) {
    ...
    const user = await this.users.findOne({ where: { stripeCustomerId: customerId } });
    if (user) {
