...
let ResumeParserService = class ResumeParserService {
    parseResumeText(text) {
        ...
        const nameCandidate = findFirst((s, i) => i < 5 &&
        ...
        if (nameCandidate) {
            const resumoProfissionalIdx = lines.findIndex(l => /^(resumo\s+profissional|perfil\s+profissional|objetivo|perfil)/i.test(l));
            ...
            if (!headlineCandidate) {
                headlineCandidate = findFirst((s, i) => i > nameCandidate.index &&
        ...
        const maritalPatterns = /(solteir[oa]|casad[oa]|viúv[oa]|divorciad[oa]|uni[ãa]o\s+est[áa]vel)/i;
        const maritalMatch = lines.find((l) => maritalPatterns.test(l));
        if (maritalMatch) {
            const match = maritalMatch.match(maritalPatterns);
            if (match)
                result.maritalStatus = match[0];
        }
        const cityPatterns = [
        ...
        const websiteMatch = text.match(/https?:\/\/(?!linkedin|github)[a-z0-9.-]+\.[a-z]{2,}/i);
        if (websiteMatch)
    ...
    parseExperiences(lines, text, mapMonth) {
        const experiences = [];
        const roleKeywords = [
        ...
        const expHeaderIdx = lines.findIndex((l) => /^(experi[eê]ncia\s+profissional|hist[óo]rico\s+profissional|atua[çc][ãa]o\s+profissional|trabalhos?\s+realizados?)$/i.test(l.trim()));
        const startIdx = expHeaderIdx >= 0 ? expHeaderIdx + 1 : 0;
        const endIdx = lines.findIndex((l, i) => i > startIdx &&
            /^(forma[cç][aã]o|educa[cç][aã]o|escolaridade|habilidades|compet[eê]ncias|cursos)/i.test(l));
        ...
        for (let i = startIdx; i < searchEnd && experiences.length < 15; i++) {
            ...
            if (!isRole &&
                /^[A-ZÁÀÂÃÉÈÊÍÏÓÔÕÖÚÇ][a-záàâãéèêíïóôõöúçñ\s]+(\s+(de|em|na|no)\s+)/i.test(line)) {
                isRole = true;
            }
            if (isRole) {
                ...
                for (const contextLine of allContextLines) {
                    if (company)
                        break;
                    if (roleKeywords.some(rx => rx.test(contextLine)))
                        continue;
                    if (/\b(19|20)\d{2}\b/.test(contextLine))
                        continue;
                    if (contextLine.length < 3 || contextLine.length > 150)
                        continue;
                ...
                const dateFormats = [
                    {
                        pattern: /(jan|fev|mar|abr|mai|jun|jul|ago|set|out|nov|dez)[a-z]*\.?\s*\/?\s*(\d{4})/gi,
                        parse: (matches) => ({
                            start: matches[0]
                ...
                for (let j = i + 1; j < Math.min(i + 6, searchEnd); j++) {
                    ...
                    if (roleKeywords.some((rx) => rx.test(descLine)))
                        break;
                    if (/^(forma[cç][aã]o|educa[cç][aã]o|compet[eê]ncias)/i.test(descLine))
                        break;
                    if (companyPatterns.some((p) => p.test(descLine)))
                        continue;
    ...
    parseEducation(lines, text, mapMonth) {
        const education = [];
        const eduHeaderIdx = lines.findIndex((l) => /(forma[cç][aã]o|educa[cç][aã]o|acad[eê]mica|escolaridade|gradua[cç][aã]o|cursos)/i.test(l));
        if (eduHeaderIdx < 0)
            return undefined;
        const startIdx = eduHeaderIdx + 1;
        const endIdx = lines.findIndex((l, i) => i > startIdx &&
            /^(experi[eê]ncia|profissional|habilidades|compet[eê]ncias|objetivo)/i.test(l));
        const searchEnd = endIdx > 0 ? endIdx : Math.min(startIdx + 30, lines.length);
        ...
        for (let i = startIdx; i < searchEnd && education.length < 10; i++) {
            ...
            if (isDegree && degree) {
                ...
                const dateContext = contextLines.join(' ');
                if (/(conclu[ií]do|formado|completo)\s+(em\s+)?(\d{4})/i.test(dateContext)) {
                    const match = dateContext.match(/(conclu[ií]do|formado|completo)\s+(em\s+)?(\d{4})/i);
                    if (match) {
    ...
    parseSkills(lines, text) {
        const skills = [];
        const skillSet = new Set();
        const skillHeaderIdx = lines.findIndex((l) => /(compet[eê]ncias|habilidades|skills|conhecimentos|ferramentas|tecnologias|idiomas|qualifica[cç][oõ]es)/i.test(l));
        if (skillHeaderIdx >= 0) {
            const startIdx = skillHeaderIdx + 1;
            const endIdx = lines.findIndex((l, i) => i > startIdx &&
                /^(experi[eê]ncia|forma[cç][aã]o|educa[cç][aã]o|objetivo|resumo)/i.test(l));
            const searchEnd = endIdx > 0 ? endIdx : Math.min(startIdx + 20, lines.length);
